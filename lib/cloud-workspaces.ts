...
function normalizeOwner(workspace: PersonalWorkspace, ownerId: string): PersonalWorkspace {
...
export async function pushWorkspaceToCloud(workspace: PersonalWorkspace) {
  ...
  const payload = normalizeOwner(workspace, user.id);
  const response = await supabaseRest("workspaces?on_conflict=id", {
    method: "POST",
...
export async function fetchCloudWorkspaces(): Promise<PersonalWorkspace[]> {
  ...
  const response = await supabaseRest(
    `workspaces?select=id,owner_id,payload,updated_at&owner_id=eq.${encodeURIComponent(user.id)}&order=updated_at.desc`,
  );
  if (!response.ok) throw new Error("Could not load synced workspaces.");
  const rows = (await response.json()) as CloudWorkspaceRow[];
  return rows
    .map((row) => row.payload)
    .filter((workspace): workspace is PersonalWorkspace => Boolean(workspace?.id && workspace?.transition))
    .map((workspace) => normalizeOwner(workspace, user.id));
}
...
export async function hydrateAuthenticatedWorkspaces() {
  ...
  previousLocal.forEach((workspace) => {
    const migrated = normalizeOwner(workspace, user.id);
  ...
  for (const workspace of merged) {
    ...
    await pushWorkspaceToCloud(workspace);
  }
