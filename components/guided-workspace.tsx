...
export function GuidedWorkspace() {
  ...
  const aiContextSources = useMemo(() => originalSources.filter((source) => source.kind === "ai-context"), [originalSources]);
  ...
  if (!workspace || !transition) {
    return <div className="min-h-screen bg-background px-5 py-20 text-center text-foreground"><h1 className="text-2xl font-semibold">No handoff selected.</h1><p className="mt-2 text-sm text-muted">Choose an existing handoff or create a new one.</p><div className="mt-6 flex justify-center gap-2"><Link href="/handoffs" className="rounded-lg border border-border px-4 py-2.5 text-sm text-muted">My handoffs</Link><Link href="/new" className="rounded-lg bg-accent px-4 py-2.5 text-sm font-medium text-white">New handoff</Link></div></div>;
  }
      ...
      <div className="mx-auto max-w-5xl px-5 pb-20 pt-7 lg:px-8 lg:pt-10">
        ...
        <motion.main key={stage} initial={reducedMotion ? false : { opacity: 0.72, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={reducedMotion ? { duration: 0 } : { duration: 0.2 }} className="min-h-[620px]">
          ...
          {stage === "handoff" && interviewComplete && (
            <section className="mx-auto max-w-3xl">
              {handoffAccepted ? <div className="rounded-3xl border border-ok/25 bg-card p-8 text-center shadow-sm sm:p-10"><motion.span initial={reducedMotion ? false : { scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 320, damping: 22 }} className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-ok/10 text-ok"><IconCheck className="h-6 w-6" /></motion.span><p className="mt-5 text-xs font-medium uppercase tracking-[0.12em] text-ok">Handoff complete</p><h1 className="mt-2 text-3xl font-semibold tracking-[-0.045em]">{transition.successor} has accepted the handoff.</h1><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-muted">The evidence was collected, the reconstruction was reviewed, the gap review was finished, and the successor explicitly verified the transfer.</p><div className="mt-7 flex flex-wrap justify-center gap-3"><Link href="/handoffs" className="h-11 rounded-lg bg-accent px-5 py-3 text-sm font-medium text-white">Back to my handoffs</Link><button type="button" onClick={() => setShowAsk((value) => !value)} className="inline-flex h-11 items-center gap-2 rounded-lg border border-border px-5 text-sm font-medium text-muted"><IconSpark className="h-4 w-4" />Ask about this handoff</button><button onClick={async () => { await navigator.clipboard.writeText(handoff); setMessage("Handoff copied as Markdown."); }} className="h-11 rounded-lg border border-border px-5 text-sm font-medium text-muted">Copy handoff record</button></div>{showAsk && <div className="mt-7 text-left"><EmbeddedAskPanel workspace={workspace} onClose={() => setShowAsk(false)} heading="Ask this completed handoff" /></div>}</div> : <>
