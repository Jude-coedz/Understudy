...
export function Shell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("understudy:identity-changed", refresh);
  }, []);
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-60 flex-col border-r border-border bg-sidebar/95 backdrop-blur-xl md:flex">
        ...
        <nav className="flex-1 space-y-1 px-3">
          {PRIMARY_NAV.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`group relative flex h-10 items-center gap-3 rounded-[10px] px-3 text-[14px] ${active ? "bg-accent-soft font-medium text-foreground" : "text-muted hover:bg-card hover:text-foreground"}`}>
