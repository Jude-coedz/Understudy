...
function statusFor(workspace: PersonalWorkspace) {
  ...
  if (unreviewed.length) return `Reviewing evidence · ${unreviewed.length} left`;
  if (critical.length) return `Filling gaps · ${critical.length} critical`;
  if (workspace.successorReview?.status === "accepted") return "Handoff accepted";
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border bg-background/95 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-5 lg:px-8">
          <Link href="/handoffs" className="flex items-center gap-2.5 text-sm font-medium tracking-[-0.02em]"><span className="flex h-8 w-8 items-center justify-center rounded-[10px] bg-foreground text-xs font-semibold text-white">U</span>Understudy</Link>
      ...
      <main className="mx-auto max-w-5xl px-5 py-9 lg:px-8 lg:py-12">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-medium uppercase tracking-[0.12em] text-subtle">Your handoffs</p>
            <h1 className="mt-2 text-3xl font-semibold tracking-[-0.045em] sm:text-4xl">One place for every transition.</h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted">Each person gets a separate evidence set, reconstruction, interview, and successor review. Switching handoffs never mixes their sources.</p>
          </div>
          <Link href="/" className="inline-flex h-11 shrink-0 items-center justify-center gap-2 rounded-lg bg-accent px-4 text-sm font-medium text-white"><IconPlus /> New handoff</Link>
        </div>
        ...
        {sorted.length ? (
                ...
                >
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {accepted && <span className="inline-flex items-center gap-1 rounded-full bg-ok/10 px-2 py-0.5 text-[11px] font-medium text-ok"><IconCheck className="h-3 w-3" />Accepted</span>}
                    </div>
                  ...
                  <div className="sm:text-right"><p className="text-sm font-medium text-muted">{statusFor(workspace)}</p><p className="mt-1 text-xs text-subtle">{workspace.transition.sources.filter((source) => source.kind !== "interview").length} evidence source{workspace.transition.sources.filter((source) => source.kind !== "interview").length === 1 ? "" : "s"}</p></div>
          ...
          <div className="mt-10 rounded-2xl border border-dashed border-border-strong bg-card p-10 text-center">
            ...
            <Link href="/" className="mt-5 inline-flex h-11 items-center gap-2 rounded-lg bg-accent px-4 text-sm font-medium text-white"><IconPlus /> Create first handoff</Link>
          </div>
        )}
      </main>
    </div>
