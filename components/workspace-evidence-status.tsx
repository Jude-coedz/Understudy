...
export function WorkspaceEvidenceStatus() {
  ...
  return (
    <div className="mx-auto max-w-[1320px] px-5 pt-5 lg:px-8">
      {unreviewed.length > 0 && (
        <div className="rounded-xl border border-warning/25 bg-warning/5 p-4">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button type="button" onClick={openReconstruction} className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-lg bg-foreground px-4 text-sm font-medium text-background">
      ...
      <div className={`${unreviewed.length ? "mt-3" : ""} rounded-xl border border-border bg-card`}>
        <button type="button" onClick={() => setCoverageOpen((value) => !value)} className="flex w-full items-center justify-between gap-4 p-4 text-left">
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <span className={`text-subtle transition-transform ${coverageOpen ? "rotate-90" : ""}`}><IconChevronRight /></span>
          </div>
        </button>
        ...
        {coverageOpen && (
          <div className="border-t border-border px-4 pb-4 pt-3">
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-lg border border-ok/20 bg-ok/5 p-3">
