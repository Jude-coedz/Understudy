...
export function HandoffRecordPage() {
  ...
  if (!workspace) {
    return <div className="min-h-screen bg-background px-5 py-20 text-center text-foreground"><h1 className="text-2xl font-semibold">No handoff selected.</h1><p className="mt-2 text-sm text-muted">Open a handoff first.</p><Link href="/" className="mt-5 inline-flex h-10 items-center rounded-lg bg-accent px-4 text-sm font-medium text-white">My handoffs</Link></div>;
  }
    ...
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-5 lg:px-8">
          ...
          <div className="flex gap-2"><Link href="/workspace" className="rounded-lg border border-border bg-card px-3 py-2 text-xs font-medium text-muted">Open handoff</Link><Link href="/handoffs" className="rounded-lg border border-border bg-card px-3 py-2 text-xs font-medium text-muted">My handoffs</Link></div>
        </div>
      </header>

      <motion.main initial={reducedMotion ? false : { opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-4xl px-5 py-10 lg:px-8 lg:py-14">
